...
const ChatView = () => {
  ...
  const options = ['Writer', 'Images' , 'PHP only']
  const [selected, setSelected] = useState(options[0])
  const [messages, addMessage, , , setLimit] = useContext(ChatContext)
  const email =  'user@email.com'
  const picUrl =  'https://img.icons8.com/pastel-glyph/256/gender-neutral-user.png'
  ...
  const sendMessage = async (e) => {
    ...
    const PATH = aiModel === options[0] ? 'davinci' :
            aiModel === options[1] ? 'dalle' :
            'php';
  ...
  return (
    <div className="chatview">
      ...
      <form className='form gap-4' onSubmit={sendMessage}>
        <select value={selected} onChange={(e) => setSelected(e.target.value)} className="dropdown border-none" >
